...
class XCLIP(CLIP4ClipPreTrainedModel):
    def __init__(self, cross_config, clip_state_dict, task_config):
        ...
        if hasattr(task_config, 'use_adapter') and task_config.use_adapter:
            adapter_config = {
                "use_lorm": True,
                "lorm_rank": getattr(task_config, 'lorm_rank', 3),
                "lorm_dropout": getattr(task_config, 'lorm_dropout', 0.0),
                "max_frames": task_config.max_frames,
                "use_asa": True,
                "asa_topk_fixed": getattr(task_config, 'asa_topk_fixed', 3),
                "asa_max_off": getattr(task_config, 'asa_max_off', 2)
            }
            show_log(task_config, "Using RAP adapters (LoRM & ASA)")
            show_log(task_config, "\t adapter_config: {}".format(adapter_config))
        self.clip = CLIP(
            embed_dim,
    ...
    def get_visual_output(self, video, video_mask, shaped=False, video_frame=-1, text_global=None):
        if shaped is False:
            video_mask = video_mask.view(-1, video_mask.shape[-1])
            video = torch.as_tensor(video).float()
            b, pair, bs, ts, channel, h, w = video.shape
        ...
        visual_hidden = self.clip.encode_image(video, video_frame=video_frame, text_global=text_global).float()
    ...
    def get_sequence_visual_output(self, input_ids, token_type_ids, attention_mask, video, video_mask, shaped=False, video_frame=-1):
        ...
        with torch.no_grad():
            try:
                text_global = self.clip.encode_text_global(input_ids)
            except AttributeError:
                text_global = None
        visual_output = self.get_visual_output(video, video_mask, shaped=True, video_frame=video_frame, text_global=text_global)
    ...
    def _loose_similarity(self, sequence_output, seq_features, visual_output, attention_mask, video_mask, sim_header="meanP"):
        ...
        if sim_header == "meanP":
            ...
            extended_video_mask = (1.0 - video_mask.unsqueeze(1)).to(dtype=torch.float32) * -1e4
            extended_video_mask = extended_video_mask.expand(-1, video_mask.size(1), -1)
            visual_output = visual_output.permute(1, 0, 2)
            visual_output = self.transformerClip(visual_output, extended_video_mask)
            visual_output = visual_output.permute(1, 0, 2)
            visual_output = visual_output + visual_output_original
        video_output = visual_output / visual_output.norm(dim=-1, keepdim=True).clamp_min(1e-6)
        video_output = self._mean_pooling_for_similarity_visual(video_output, video_mask)
        video_output = video_output / video_output.norm(dim=-1, keepdim=True).clamp_min(1e-6)
        if self.use_original_clip_for_frame_features:
            frame_features = visual_output_original / visual_output_original.norm(dim=-1, keepdim=True).clamp_min(1e-6)
        else:
            frame_features = visual_output / visual_output.norm(dim=-1, keepdim=True).clamp_min(1e-6)
        sentence_output = sequence_output.squeeze(1)
        sentence_output  = sentence_output / sentence_output.norm(dim=-1, keepdim=True).clamp_min(1e-6)
        word_features = seq_features / seq_features.norm(dim=-1, keepdim=True).clamp_min(1e-6)
        max_logit = math.log(100.0)
        logit_scale = torch.clamp(self.clip.logit_scale, max=max_logit).exp()
        if self.training:
    ...
    def _attenion_over_fine_grained_sim_matrix(self, word_features, frame_features):
        ...
        fine_grained_sim_scores = torch.matmul(torch.matmul(word_features.view(-1, dim_text), self.local_mat_weight), frame_features.view(-1, dim_video).t()).view(bs_text, num_words, bs_video, num_frames)
